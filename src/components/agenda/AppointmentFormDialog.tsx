...
export default function AppointmentFormDialog({ open, onOpenChange, defaultDate, defaultTime, isEncaixe = false, editAppointment }: Props) {
  ...
  useEffect(() => {
    if (editAppointment) {
      ...
      setForm(f => ({
        ...
        duration_minutes: editAppointment.duration_minutes,
  ...
  const selectPatient = (p: typeof patients extends (infer T)[] | undefined ? NonNullable<T> : never) => {
    setForm(f => ({
      ...f,
      patient_id: p.id,
      phone: p.phone || "",
        ...
        <form onSubmit={handleSubmit}>
          <Tabs defaultValue="paciente" className="w-full">
            ...
            <TabsContent value="paciente" className="space-y-4">
                ...
                <div className="p-3 bg-muted/30 rounded-lg space-y-2">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">{selectedPatient.cpf || "—"} · {selectedPatient.phone || "—"}</p>
                    </div>
                    <Button type="button" variant="ghost" size="sm" onClick={() => setField("patient_id", "")}>Trocar</Button>
                  </div>
                  <div className="grid grid-cols-3 gap-2 text-xs">
                    <div><span className="text-muted-foreground">Nascimento:</span> {selectedPatient.birth_date}</div>
                    <div><span className="text-muted-foreground">Sexo:</span> {selectedPatient.gender}</div>
                    <div><span className="text-muted-foreground">Convênio:</span> {selectedPatient.health_insurance || "—"}</div>
                  </div>
                </div>
              )}

              {isProvisional && (
                <div className="space-y-3 p-3 border border-dashed border-amber-300 rounded-lg bg-amber-50/30">
                  <div className="flex items-center justify-between">
                    <Label className="text-xs font-semibold text-amber-700">Paciente Provisório</Label>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div className="space-y-1.5">
                      ...
                      <Input value={form.responsible} onChange={e => setField("responsible", e.target.value)} placeholder="Se menor ou dependente" />
                    </div>
                  </div>
                </div>
              )}
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="flex items-end gap-4 pb-1">
                  ...
                  <label className="flex items-center gap-2 text-xs cursor-pointer">
                    <Checkbox checked={form.is_new_patient} onCheckedChange={v => setField("is_new_patient", !!v)} />Paciente Novo
                  </label>
                  <label className="flex items-center gap-2 text-xs cursor-pointer">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="space-y-1.5">
                  ...
                  <Select value={form.agenda_id} onValueChange={v => {
                    ...
                    <SelectTrigger className="h-9 text-xs"><SelectValue placeholder="Selecione a agenda" /></SelectTrigger>
                    <SelectContent>
                      {agendas?.filter(a => a.status === "ativa").map(a => (
                        <SelectItem key={a.id} value={a.id}>{a.name}{a.specialty ? ` (${a.specialty})` : ""}</SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                </div>
            ...
            <TabsContent value="horario" className="space-y-4">
              <div className="grid grid-cols-3 gap-3">
                <div className="space-y-1.5">
                  ...
                  <Input type="date" className="h-9 text-xs" value={form.scheduled_date} onChange={e => setField("scheduled_date", e.target.value)} required />
                  {errors.date && <p className="text-xs text-destructive">{errors.date}</p>}
                </div>
                <div className="space-y-1.5">
                  <Label className="text-xs">Início *</Label>
                  <Input type="time" className="h-9 text-xs" value={form.scheduled_time} onChange={e => setField("scheduled_time", e.target.value)} required />
              ...
              {selectedAgenda && (
                <div className="p-3 bg-primary/5 rounded-lg text-xs space-y-1">
                  <p className="font-semibold text-primary">Agenda: {selectedAgenda.name}</p>
                  <p className="text-muted-foreground">Intervalo padrão: {selectedAgenda.default_interval}min · Duração: {selectedAgenda.default_duration}min</p>
                  {selectedAgenda.daily_patient_limit && <p className="text-muted-foreground">Limite diário: {selectedAgenda.daily_patient_limit} pacientes</p>}
                </div>
              )}
            </TabsContent>
            ...
            <TabsContent value="outros" className="space-y-4">
              ...
              <div className="space-y-1.5">
                <Label className="text-xs">Observação do Procedimento</Label>
                <Textarea value={form.procedure_notes} onChange={e => setField("procedure_notes", e.target.value)} rows={2} className="text-xs" placeholder="Detalhes sobre o procedimento" />
              </div>
              <div className="space-y-1.5">
                <Label className="text-xs">{isEncaixe ? "Justificativa do Encaixe *" : "Observação Geral"}</Label>
                <Textarea value={form.notes} onChange={e => setField("notes", e.target.value)} rows={2} className="text-xs" placeholder={isEncaixe ? "Explique o motivo do encaixe..." : "Observações gerais"} required={isEncaixe} />
                {errors.notes && <p className="text-xs text-destructive">{errors.notes}</p>}
              </div>
              <div className="space-y-1.5">
                <Label className="text-xs">Necessidades Especiais</Label>
                <Input className="h-9 text-xs" value={form.special_needs} onChange={e => setField("special_needs", e.target.value)} placeholder="Ex: Cadeira de rodas, intérprete" />
              </div>
              <div className="space-y-1.5">
                <Label className="text-xs">Observação Administrativa</Label>
                <Textarea value={form.admin_notes} onChange={e => setField("admin_notes", e.target.value)} rows={2} className="text-xs" placeholder="Visível apenas internamente" />
              </div>
              <label className="flex items-center gap-2 text-xs cursor-pointer">
                <Checkbox checked={form.reminder} onCheckedChange={v => setField("reminder", !!v)} />Enviar lembrete ao paciente
              </label>
            </TabsContent>
          </Tabs>
          ...
          <div className="flex justify-end gap-3 pt-4 mt-4 border-t">
            ...
            <Button type="submit" disabled={createAppointment.isPending || updateAppointment.isPending}>
              ...
              {editAppointment ? "Salvar Alterações" : isEncaixe ? "Salvar Encaixe" : "Criar Agendamento"}
            </Button>
          </div>
        </form>
      </DialogContent>
